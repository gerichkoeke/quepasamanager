...
const webhookTarget = `    // Only process outgoing messages (agent responses)
    if (messageType !== 'outgoing') {
      logger.debug({ messageType }, 'Ignoring non-outgoing message');
      return res.json({ success: true, message: 'Not an outgoing message' });
    }`;

const webhookLogic = `    // Check for Typebot manual triggers
...
code = code.split(webhookTarget).join(webhookLogic);
